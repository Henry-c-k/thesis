...
# lam = 12 # intensity of jump i.e. number of jumps per annum
# steps =30 # time steps
# Npaths = 50000 # number of paths to simulate
...
def merton_jump_call(S, K, T, r, v, lam):
    p = 0
    sig  = 0.2
    for k in range(40):
        r_k = r - lam*(np.exp(v**2*0.5)-1) + (k*np.log(np.exp(v**2*0.5)) ) / T
        sigma_k = np.sqrt( sig**2 + (k* v** 2) / T)
        k_fact = np.math.factorial(k)
        p += (np.exp(-np.exp(v**2*0.5)*lam*T) * (np.exp(v**2*0.5)*lam*T)**k / (k_fact))  * BS_CALL(S, K, T, r_k, sigma_k)
...
x0 = [0.1, 1] # m(mu), v(delta), lambda
bounds = ((1e-5, np.inf) , (0, 20)) #bounds as described above
...
print('Calibrated Volatlity = ', 0.2)
print('Calibrated Jump Std = ', vt)
print('Calibrated Jump Mean = ',0.0)
